...
nlm0 <- function(fun,range,prec=1e-7)
{
    ...
    if (uu.x-ll.x<prec) {
        sol <- (ll.x+uu.x)/2
        fit <- fun(sol)
        print(paste("lower and upper lambda range is small: ll =",ll.x,", uu.x =",uu.x))
    ...
    ## Initialization
    uu.fit <- fun(uu.x)
    print(paste("cv(upper lambda) =",fun(uu.x)))
    mu.fit <- fun(mu.x)
    print(paste("cv(mid-upper lambda) =",fun(mu.x)))
    ml.fit <- fun(ml.x)
    print(paste("cv(mid-lower lambda) =",fun(ml.x)))
    ll.fit <- fun(ll.x)
    print(paste("cv(lower lambda) =",fun(ll.x)))
    neval <- 4
    ## Iteration
    repeat {
